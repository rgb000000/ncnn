//
// Created by rgb000000 on 4/28/22.
//
...
void print_mat_shape(const ncnn::Mat& m){
    ncnn::Mat shape = m.shape();
    int c = shape.c;
    int h = shape.h;
    int w = shape.w;
    fprintf(stderr, "(c, h, w)=(%d, %d, %d)\n", c, h, w);
}

void print_mat_w(const ncnn::Mat& m, int w){
    const float* row = m.row(w);
    int len = m.w;
    fprintf(stderr, "len = %d, m[:, :, %d]: \n", len, w);
    for(int i=0; i<len; i++){
        fprintf(stderr, "%f ", row[i]);
    }
    fprintf(stderr, "\n");
}
...
void detect_object(const cv::Mat& im, std::vector<Object>& objects){
...
//    yolov3_tiny.load_model("/home/PRJ/mycpu/ncnn/build/examples/yolov3-tiny_train_opt_int8.bin");
    ...
    ncnn::Mat in = ncnn::Mat::from_pixels_resize(im.data, ncnn::Mat::PIXEL_BGR2RGB, img_w, img_h, w, h);
    print_mat_shape(in);
...
//    print_mat_shape(out1);
//    print_mat_shape(out2);
//    print_mat_w(out1, 0);
    ...
    // stride    3x1600x85
    {
        ...
        generate_proposals(anchors, 16, in_pad, out1, prob_threshold, objects8);
    ...
    // stride  3x400x85
    {
        ...
        std::vector<Object> objects8;
        generate_proposals(anchors, 32, in_pad, out2, prob_threshold, objects8);
